...
;; Replacing the first occurence of the variable tracker by the given variable value.
...
;;This function will check if the ball hit one of the moving platform
  ...
  (cond
    ...
    [(and
      (<= (ball-x state) (- (+ (image-width BAR) (/ (image-width BALL) 2)) (* 0.5 (image-width BAR)))) 
       (<= (cond
        [( >= (left-bar-y state) (ball-y state)) ( - (left-bar-y state) (ball-y state))]
        [else ( - (ball-y state) (left-bar-y state))]) (+ (/ (image-height BAR) 2) (/ ERROR-MARGIN 2)))) #t]
    [(and
      (<= (ball-x state) (- (+ (image-width BAR) (/ (image-width BALL) 2)) (* 0.5 (image-width BAR))))
      (equal? (cond
        [( >= (left-bar-y state) (ball-y state)) ( - (left-bar-y state) (ball-y state))]
      ...
      (move-ball
       (cond
         ...
         [(or (equal? (ball-y state) BOUNDARY)
          ...
          (replace state THREE (* (sub1 ZERO) (ball-y-vec state)))]
         [else state]))))
...
(define (arti-intel state)
  (replace state FIVE (ball-y state)))
